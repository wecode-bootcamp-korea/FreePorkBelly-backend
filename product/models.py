...
class Product(models.Model):
    ...
    unit_price_comment = models.CharField(max_length=300, null=True)                 # 상품별로 단위가 달라서 단위가격 코멘트가 다 상이함
    unit_price = models.DecimalField(max_digits=10, decimal_places=2, null=True)
    sales_price_comment = models.CharField(max_length=300, null=True)                # 상품별로 판매단위가 다 달라 판매가격 코멘트가 다 상이함
    sales_price = models.DecimalField(max_digits=10, decimal_places=2, null=True)
    main_img_url = models.URLField(max_length=2000, null=True)   # 상품 디테일 화면에 뿌려지는 black 배경 이미지 
    sub_img_url = models.URLField(max_length=2000, null=True)    # 카테고리에서 뿌려지는 white 배경 이미지
    ...
    information = models.CharField(max_length=2000, null=True)  # 상품 기본정보 -> 실제 반영할지는 추후 판단함
...
class ProductDescription(models.Model):
    ...
    photo_order = models.IntegerField(null=True)
